...
class UrMoveClient:
    ...
    def __init__(self, 
        ...
        Args:
            server_address: ur_move规划服务器地址（规划PC）
    ...
    def plan_and_execute_remote(self, waypoints: Dict[str, Any]) -> Dict[str, Any]:
        """
        规划轨迹并在远程驱动PC上执行
        ...
        # 步骤2: 在远程驱动PC上执行轨迹
        logger.info("步骤2: 在远程驱动PC上执行轨迹...")
        execution_result = self.execute_remote(plan_result)
    ...
    def execute_remote(self, plan_result: Dict[str, Any]) -> Dict[str, Any]:
        """
        在远程驱动PC上执行已规划的轨迹
        ...
        try:
            ...
            # 执行所有轨迹
            execution_results = executor_client.execute_trajectories(filtered_trajectories)
            
            # 检查是否所有执行都成功，并生成消息
            all_success = all(result.get("success", False) for result in execution_results.values())
